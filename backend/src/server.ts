...
export function createServer(db: Database.Database) {
  ...
  app.post("/api/screenplays", (req, res) => {
    ...
    const now = new Date().toISOString();
    db.prepare(
      "INSERT INTO screenplays (id, user_id, title, createdAt, updatedAt) VALUES (?,?,?,?,?)"
    ).run(id, userId, title ?? "", now, now);
    ok(res, { id, userId, title });
  });
  ...
  app.delete("/api/screenplays/:id", (req, res) => {
    db.prepare("DELETE FROM screenplays WHERE id=?").run(req.params.id);
    ok(res, true);
  });

  app.get("/api/screenplays/:id/content", (req, res) => {
    const row = db
      .prepare("SELECT html, updatedAt FROM screenplay_content WHERE screenplayId=?")
      .get(req.params.id);
  ...
  app.put("/api/screenplays/:id/content", (req, res) => {
    ...
    const stmt = db.prepare(
      "INSERT INTO screenplay_content (screenplayId, html, updatedAt) VALUES (?, ?, ?) ON CONFLICT(screenplayId) DO UPDATE SET html=excluded.html, updatedAt=excluded.updatedAt"
    );
  ...
  app.post("/api/screenplays/:id/characters", (req, res) => {
    ...
    db.prepare("INSERT INTO characters (id, screenplayId, name, role) VALUES (?,?,?,?)").run(
      id,
      req.params.id,
      name ?? "\u0628\u062f\u0648\u0646 \u0627\u0633\u0645",
      role ?? ""
  ...
  app.get("/api/users/:userId/sprints/active", (req, res) => {
    const row = db
      .prepare("SELECT * FROM sprints WHERE userId=? AND isActive=1 ORDER BY startedAt DESC LIMIT 1")
  ...
  app.post("/api/users/:userId/sprints", (req, res) => {
    ...
    db.prepare("INSERT INTO sprints (id, userId, isActive, startedAt) VALUES (?,?,1,?)").run(
      id,
      req.params.userId,
      startedAt
    );
  ...
  app.put("/api/users/:userId/sprints/:sprintId", (req, res) => {
    ...
    if (action === "end") {
      db.prepare("UPDATE sprints SET isActive=0, endedAt=?, durationSec=? WHERE id=? AND userId=?").run(
      ...
      db.prepare("UPDATE sprints SET isActive=0 WHERE id=? AND userId=?").run(
        req.params.sprintId,
        req.params.userId
      );
    }
  ...
  app.post("/api/users/:userId/stash", (req, res) => {
    ...
    db.prepare(
      "INSERT INTO stash (id, userId, text, type, wordCount, createdAt) VALUES (?,?,?,?,?,?)"
  ...
  app.post("/api/export/pdf", async (req, res) => {
    try {
      ...
      await page.setContent(
        `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"/>${style}</head><body><div class="screenplay-page">${html}</div></body></html>`,
      ...
      const pdf = await page.pdf({
        ...
        footerTemplate: `<div style="font-size:10px; width:100%; padding:0 1in; direction:rtl;"><div style="text-align:center; width:100%;"><span class="pageNumber"></span> / <span class="totalPages"></span></div></div>`,
      });
      await browser.close();
